...
The frontend applications for the customers and the employees were developed by other students and as such are not provided.
<br/><br/>
...
- Included Testcontainers Oracle test database (dev profile)
- Full Swagger API documentation (Polish only)
...
### Prerequisites
- Java, ver. 17
- Docker Desktop (DEMO Mode) OR Oracle DB wallet (Production Mode)
...
You can however run the program in DEMO mode where a temporary test database will be created for You using docker (Image size ~2GB).
- Install the newest .jar file from the „Releases” section of this GitHub page
- Run the packaged .jar
  - DEMO mode - ensure Docker Desktop is running
  ```
  java -jar NAME_OF_THE_JAR_FILE.jar --spring.profiles.active=dev
  ```
  - Production use with Oracle DB using wallets (place wallet files in /wallet folder in the same folder as jar) 
  ```
...
```
In DEMO mode You use following credentials:
- Example customer: piotr.kasprzak@example.com, p123
- Example employee: jan.kowalski@example.com, haslo123
